...
mod decision_tree {
    ...
    fn boston_tree_basic_test() {
        ...
        assert!(
            ...
            "Prediction out of plausible range"
        );

        // Test variance analysis
        let gains = model.analyze_gains();
        for &g in &gains {
            assert!(g >= 0.0, "Negative variance gain found");
        }
        ...
        assert!(
            cumulative <= total_sse,
            "Cumulative variance gain exceeds total SSE"
        );
        assert!(
            cumulative / total_sse > 0.65,
            "Total Explained Variance too low -> degredation"
        );
        assert!(
            cumulative / total_sse < 0.98,
            "Total Explained Variance too high to be realistic"
        );
    }
    #[test]
