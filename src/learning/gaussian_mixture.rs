...
use crate::random::generation::{generate_random_vector, generate_zero_matrix};
...
pub fn kmeans_gmm_pipeline(
    ...
    let mut gmm = GaussianMixtureModel {
        ...
        means: kmeans.means,
        variance: (0..centroids)
            .map(|_| create_identity_matrix(cardinality))
            .collect(),
    };
    gmm.solve(data);
    gmm
    ...
    pub fn expectation_maximization(&mut self, data: &[Vec<f32>]) {
        ...
        for x_i in data {
            ...
            for k in 0..self.centroids {
                ...
                probs[k] =
                    self.mixtures[k].ln() + ln_gaussian(&mut x_bar, &mut z_buf, dets[k], &lus[k]);
        ...
        for k in 0..self.centroids {
            ...
            for i in 0..self.cardinality {
                for j in 0..=i {
                    ...
                    sum_squares[k].data[i * self.cardinality + j] -=
                        sum_linear[k][i] * sum_linear[k][j];
                    sum_squares[k].data[j * self.cardinality + i] =
                        sum_squares[k].data[i * self.cardinality + j];
                }
            }
        }
