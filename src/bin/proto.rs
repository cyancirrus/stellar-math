...
fn quick_outer_product(reflection:HouseholderReflection) -> NdArray {
    ...
    for i in 0..length {
        for j in 0..length {
            ndarray.data[i * length + j ] -= reflection.beta * reflection.vector[i] * reflection.vector[j];
        }
    }
    ndarray
}
...
fn main() {
    let x = HouseholderReflection { beta: 0.8038838, vector: vec![-1.2198039, 1.0] };
    let result = quick_outer_product(x);
    println!("outer product for result {:?}", result);
    // // {
    //     // Eigen values 2, -1
