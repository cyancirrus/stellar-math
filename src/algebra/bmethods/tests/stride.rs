...
mod test_stride_behavior {
    ...
    fn test_sfma_equivalence() {
        ...
        for (i, k, j) in test_data() {
            let s_x = ds_x + k;
            let s_y = ds_y + j;
            // sfma_matmul_equivalence(i, k, j, s_x, s_y);
            // sfma_tmatmul_equivalence(i, k, j);
            // sfma_lower_equivalence(i, k, j);
            // sfma_upper_equivalence(i, k, j);
            // sfma_rlower_equivalence(i, k, j);
            // sfma_rupper_equivalence(i, k, j);
            // sfma_ltl_equivalence(i, k, j);
            // sfma_ltu_equivalence(i, k, j);
            // sfma_ltl_equivalence(i, k, j);
        }
    }
    fn sfma_matmul_equivalence(m: usize, p: usize, n: usize, s_x: usize, s_y: usize) {
    ...
    fn sfma_tmatmul_equivalence(m: usize, p: usize, n: usize) {
        let y = generate_random_matrix(p, n);
        ...
        // tensor_tkernel(&x.data, &y.data, &mut t_d, m, p, n, m, n, n);
    ...
    fn sfma_lower_equivalence(m: usize, p: usize, n: usize) {
        let x = generate_random_matrix(m, p);
        let y = generate_random_matrix(p, n);
        let mut t_d = generate_random_vector(m * n);
        ...
        let mut expected = basic_mult(&x_base, &y);
        increment(&mut expected.data, &t_d, m, n, n, n);
        // tensor_lt_block(&x.data, &y.data, &mut t_d, m, p, n, p, n, n);
