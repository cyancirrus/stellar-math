...
use crate::decomposition::francis::primitives::{
    complex_eig_pair, deflate, eigen, double_shift, exception_shift, lapply_householder, params,
    rapply_householder,
};
...
fn decomp_cpx(h: &mut [f32], w: &mut [f32], mut range: usize, size: usize, stride: usize) {
    let s = range * stride;
...
fn decomp_sym(h: &mut [f32], mut range: usize, size: usize, stride: usize) {
    let s = range * stride;
    let mut e1 = s.saturating_sub(stride + 1);
    let mut e2 = s.saturating_sub(stride + stride + 2);
    let mut tl = s.saturating_sub(stride + 2);
